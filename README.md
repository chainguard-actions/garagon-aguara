...
```bash
curl -fsSL https://raw.githubusercontent.com/garagon/aguara/main/install.sh \
  | VERSION=v0.22.2 INSTALL_DIR=/usr/local/bin sh
```
...
- name: Scan for security issues
  run: docker run --rm -v "${{ github.workspace }}:/scan:ro" ghcr.io/garagon/aguara:0.22.2 scan /scan --ci
```
...
Aguara Watch is being reworked. The previous public observatory is stale, so it is not a supported product surface for v0.22.2. The supported surfaces are the CLI, GitHub Action, Docker image, signed releases, and Go library.
